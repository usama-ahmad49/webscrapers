...
cs = open('shadesdaddy.csv', 'w', newline="", encoding='utf-8')
...
class shadesdaddy(scrapy.Spider):
    name='shadesdaddy'

    def start_requests(self):
        urls=['https://shadesdaddy.com/collections/women-sunglasses?page=1','https://shadesdaddy.com/collections/men-sunglasses?page=1']
        for url in urls:
            count = 2
            driver = webdriver.Chrome()
            driver.get(url)
            time.sleep(4)
            pages = int(driver.find_element_by_css_selector('#shopify-section-collection-template > section:nth-child(2) > div > div:nth-child(1) > div > div > span.boost-pfs-filter-top-pagination').text.split()[-1])
            driver.close()
            if 'women' in url:
                gender='Women'
            else:
                gender='Men'
            yield scrapy.Request(url=url,callback=self.parse,meta={'gender':gender,'pages':pages,'count':count})

    def parse(self, response):
        for link in response.css('div.boost-pfs-filter-products.product-list.is-flex.is-flex-wrap.equal-columns--outside-trim div.relative.product_image a::attr(href)').getall():
            yield scrapy.Request(url='https://shadesdaddy.com'+link,callback=self.next,meta={'gender':response.meta['gender']})

        if response.meta['count']<=response.meta['pages']:
            page_link=response.url.split('?')[0]
            yield scrapy.Request(url=page_link+'?page='+str(response.meta['count']),callback=self.parse,meta={'gender':response.meta['gender'],'pages':response.meta['pages'],'count':response.meta['count']+1})

    def next(self,response):
        item=dict()
        json_string=response.css('script[type="application/json"]::text').get()
        json_data=json.loads(json_string)
        item['ref']=ts
        item['brand']=json_data['vendor']
        item['main_pic']=json_data['featured_image']
        item['gender']=response.meta['gender']
        item['main_category']=item['gender']
        item['category']='Glasses'
        item['sub_category']='Sunglasses'
        item['name_en']=json_data['title']
        item['group_sku']=response.url
        item['original_price']=response.css('div.modal_price span.current_price span span.money::text').get()[1:]
        item['retail_price']=item['original_price']
        item['whole_sale_price']=item['original_price']
        if response.css('ul.tabs-content li.active p::text').get() is not None:
            item['description_en']=response.css('ul.tabs-content li.active p::text').get()
            item['description_plain_en']=item['description_en']

        try:
            item['color']=json_data['variants'][0]['featured_image']['alt']
        except:
            item['color'] = ''
            b=1
        try:
            item['product_type'] = json_data['variants'][0]['sku']+'-'+item['color']
        except:
            if item['color'] == None:
                item['product_type'] = json_data['variants'][0]['sku']
        item['size_slug']='EU'
        item['origin']='Website'
        item['delivery_information']='10 15 Days'
        if response.css('div.modal_price span.was_price span span.money::text').get() is not None:
            item['strike_through_price']=response.css('div.modal_price span.was_price span span.money::text').get()[1:]
        ...
        for var in json_data['variants']:
            item[f'variant_{index}'] = var['title']
            item[f'variant_{index}_price'] = var['price']/100
