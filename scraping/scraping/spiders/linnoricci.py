...
cs = open('linoricci.csv', 'w', newline="", encoding='utf-8')
...
class linoricci(scrapy.Spider):
    name='linoricci'
    # start_urls=['https://www.linoricci.com/en/16-bags-and-luggages','https://www.linoricci.com/en/17-shoes','https://www.linoricci.com/en/58-sunglasses','https://www.linoricci.com/en/135-bags-and-luggages','https://www.linoricci.com/en/81-shoes']
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=url, meta={'main_category': 'men', 'category': 'sunclasses'})
        '''men bags'''
    ...
    def parse(self, response):
        for i in response.css('section#products article div.product-image a::attr(href)').getall():
            yield scrapy.Request(url=i,callback=self.final_page,meta={'main_category':response.meta['main_category'],'category':response.meta['category']})

        if response.css('nav.pagination a[rel=next]::attr(href)').get():
            # print(response.css('nav.pagination a[rel=next]::attr(href)').get())
    ...
    def final_page(self,response):
        ...
        item['ref']=ts
        item['original_price']=float(response.css('div.product-prices div.current-price span[itemprop=price]::text').get()[1:].replace(',',''))
        ...
        # item['group_sku'] = item['group_sku'].replace(' ','-')
        # item['product_type'] = sku+'-'+item['color']
        item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', item['group_sku'])
        item['gender']=response.meta['main_category']
        # item['main_category']=response.css('div.box-breadcrumb a span::text').getall()[1]
