...
import scrapy
import csv
headers_csv = ['serial_no', 'ref', 'partner_id', 'product_type','SKU', 'group_sku', 'variation_type', 'product_sku', 'upc_ean', 'barcode', 'brand',
...
csvfile = open('kith.csv', 'w', newline='', encoding="utf-8")
writer = csv.DictWriter(csvfile, fieldnames=headers_csv)
writer.writeheader()

flag = True
class kith(scrapy.Spider):
    name = 'kith'
    # start_urls = ['https://kith.com/collections/mens-footwear-sneakers', 'https://kith.com/collections/womens-footwear-sneakers', 'https://kith.com/collections/kids-footwear-sneakers']
    # start_urls = ['https://kith.com/collections/mens-footwear-sneakers/products/aafy4493']
    # custom_settings = {
    #     'DOWNLOAD_DELAY': 0,
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url, meta={'gender':'mens'})
        url = 'https://kith.com/collections/womens-footwear-sneakers'
        yield scrapy.Request(url, meta={'gender':'womens'})
        url = 'https://kith.com/collections/kids-footwear-sneakers'
        yield scrapy.Request(url, meta={'gender':'kids'})

    def parse(self, response, **kwargs):
        # yield scrapy.Request(url=response.url, callback=self.scrape_Data, meta={'gender':response.meta['gender']})
        page_number = 1
        while page_number<15:
            yield scrapy.Request(url=response.url+f'?page={page_number}', callback=self.scrape_Data, meta={'gender':response.meta['gender']})
            print(response.url+f'?page={page_number}')
            page_number += 1


    def scrape_Data(self, response):
        if response.css('.collection-product') != []:
            for data in response.css('.collection-product'):
                url = 'https://kith.com'+data.css('.product-card a::attr(href)').extract_first()
                # url = 'https://kith.com/collections/mens-footwear-sneakers/products/adidas-originals-x-kinfolk-ultraboost-navy'
                yield scrapy.Request(url=url, callback=self.scrapeindividual,meta={'gender':response.meta['gender']})
        else:
            pass

    def scrapeindividual(self, response):
        item = dict()
        string = [v for v in response.css('script::text').extract() if 'window.ShopifyAnalytics = window.ShopifyAnalytics' in v][0].split('var meta =')[1].split(';')[0]
        data = json.loads(string)
        if len([v for v in response.url.split('/') if 'collections' in v])>0:
            if len(response.url.split('/')[-1].split('-')) < 4:
                item['SKU'] = response.url.split('/')[-1]
            else:
                item['SKU'] = data['product']['variants'][0]['sku']
        else:
            item['SKU'] = data['product']['variants'][0]['sku']
        item['serial_no'] = data['product']['variants'][0]['sku']
        item['brand'] = data['product']['vendor']
        item['category'] = data['product']['type']
        item['color'] = response.css('.product__color::text').extract_first()
        item['main_pic'] = response.css('.product-image-carousel__slide')[0].css('img::attr(src)').extract_first()
        item['description_en'] = ' '.join(response.css('#product-details ::text').extract()).strip()
        item['description_plain_en'] = ' '.join(response.css('#product-details ::text').extract()).strip().replace('\n','')
        item['gender'] = response.meta['gender']
        item['group_sku'] =data['product']['variants'][0]['sku']
        item['product_sku'] =data['product']['variants'][0]['sku']
        try:
            item['strike_through_price'] = float(response.css('.product__price .product-price__compare::text').extract_first().replace('$','').replace(',',''))
        except:
            pass
        item['original_price'] =float(response.css('meta[property="og:price:amount"]::attr(content)').extract_first().replace(',',''))
        item['retail_price'] =float(response.css('meta[property="og:price:amount"]::attr(content)').extract_first().replace(',',''))
        item['whole_sale_price'] =float(response.css('meta[property="og:price:amount"]::attr(content)').extract_first().replace(',',''))
        item['main_category'] = response.meta['gender']
        item['sub_category'] = data['product']['vendor']
        item['base_category'] = item['main_category'] + '/'+item['category']+'/'+item['sub_category']
        item['ref'] = response.url
        item['origin'] = "website"
        # item['main_pic'] = response.css('.product-image-carousel__slide')[0].css('img::attr(src)').extract_first()
        for i, img in enumerate(response.css('.product-image-carousel__slide')):
            if i > 36:
                break
            item[f'pic_{i + 1}'] = img.css('img::attr(src)').extract_first()
        for index, variant in enumerate(data['product']['variants']):
            item['variant_{}'.format(index + 1)] = variant['public_title']
            item['variant_{}_price'.format(index + 1)] = float(response.css('meta[property="og:price:amount"]::attr(content)').extract_first().replace(',',''))
            item['variant_{}_stock'.format(index + 1)] = 'Out Of Stock'
            for size in response.css('#SingleOptionSelector-0 option')[1:]:
                if size.css('::text').extract_first().strip() == variant['public_title']:
                    item['variant_{}_stock'.format(index + 1)] = 'In Stock'
                    break
