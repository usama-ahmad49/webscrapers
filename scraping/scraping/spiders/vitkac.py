...
cs = open('vitkac.csv', 'w', newline="", encoding='utf-8')
...
class vikac(scrapy.Spider):
    ...
    def next(self, response):
        ...
        item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', (response.css('span#productSymbol::text').get() + '-' + item['color']))
        ...
        for var in response.css('div#product_sizes ul li'):
            if index <= 100:
                item[f'variant_{index}'] = var.css('a::text').get().strip().replace(',', '.')
                item[f'variant_{index}_price'] = item['original_price']  # (30+0.83*Discount Price )*4.57
