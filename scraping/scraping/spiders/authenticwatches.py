...
cs = open('authenticwatches.csv', 'w', newline="", encoding='utf-8')
...
class authenticwatches(scrapy.Spider):
    name = 'authenticwatches'

    def start_requests(self):
        yield scrapy.Request(url='https://www.authenticwatches.com/mens-watches-specials.html#p=10000',callback=self.parse,meta={'gender':'Men'})
        yield scrapy.Request(url='https://www.authenticwatches.com/womens-watches-specials.html#p=10000',callback=self.parse,meta={'gender':'Women'})


    def parse(self, response):
        for links in response.css('div.list.flw.items.section.columns.col5 p a[itemprop="url"]::attr(href)').getall():
            yield scrapy.Request(url='https://www.authenticwatches.com/'+links,callback=self.next,meta={'gender':response.meta['gender']})

    def next(self,response):
        item=dict()
        item['ref']=ts
        item['brand']=response.css('div.pit.fr h1 i::text').get()
        item['name_en']=response.css('div.pit.fr h1::text').get()
        item['original_price']=response.css('div.sale.fl span::text').get()
        item['retail_price']=item['original_price']
        item['whole_sale_price']=item['original_price']
        if response.css('div.retail.line-through.fl span::text').get() is not None:
            item['strike_through_price']=response.css('div.retail.line-through.fl span::text').get()[1:]

        if 'strike_through_price' not in item.keys():
            item['strike_through_price']=response.css('div.past.line-through.fl span::text').get()


        item['gender']=response.meta['gender']
        item['product_type']=response.css('div.code.fl::text').get()
        item['description_en']=response.css('div.nounderline font[color="666666"]::text').get()
        ...
        item['main_pic']=response.css('div.image-container div a.MagicZoom::attr(href)').get()
        item['size_slug']='Eu'
        item['group_sku']=response.url
        item['group_sku']=response.url

        t = 1
        for j in response.css('div.magic-zoom-gallery a::attr(href)').getall():
            if t < 37:
                item['pic_' + str(t)] = j
                t = t + 1
            else:
        ...
        item['variant_1'] = item['product_type']
        item['variant_1_price'] = item['original_price']
        if response.css('div.option.instock.flw span.icon-ok::text').get() is not None:
            if 'In Stock' in response.css('div.option.instock.flw span.icon-ok::text').get():
...
if __name__ == '__main__':
    ...
    process.crawl(authenticwatches)
    process.start()


# https://www.authenticwatches.com/breitling-vb5510h1-be45.html
# https://www.authenticwatches.com/rolex-gmt-master-16713ln.html
# https://www.authenticwatches.com/rolex-yacht-master-116680-mens-watch.html