...
obj = odooApi.odooClient()

cwd = os.getcwd()
brands = ['adidas', 'Asics', 'Converse', 'Hender Scheme', 'Jordan', 'Nike']
genders = ['men', 'women', 'preschool', 'toddler', 'child', 'infant', 'unisex']
processed = []

headers_csv = ['serial_no', 'ref', 'url_key', 'partner_id', 'product_type', 'alreadyexists', 'group_sku', 'condition', 'ticker', 'StyleId',
...
csvfile = open('stockx_data.csv', 'w', newline='', encoding='utf-8')
writerstockx = csv.DictWriter(csvfile, fieldnames=headers_csv)
writerstockx.writeheader()
csvfile.flush()
# '''file for magento'''
# priceupdateheaders = ['sku', 'price', 'status', 'store_view_code']
# priceupdatemagentonew = open('stockx_pricelist_magento_new_temp.csv', 'w', newline='', encoding='utf-8')
# pricewritermagentonew = csv.DictWriter(priceupdatemagentonew, fieldnames=priceupdateheaders)
# pricewritermagentonew.writeheader()
#
# '''file for pricelist'''
# pricelistheaders = ['GROUP SKU', 'PRODUCT SKU', 'RETAIL PRICE', 'WEBSITES']
# pricelistodoonew = open('stockx_pricelist_odoo_new_temp.csv', 'w', newline='', encoding='utf-8')
# pricelistwriterodoonew = csv.DictWriter(pricelistodoonew, fieldnames=pricelistheaders)
# pricelistwriterodoonew.writeheader()

ct = datetime.datetime.now()
TS = str(ct.timestamp()).replace('.', '')

fileoutput = open('skudictionary.json','r')
linksDictionar = json.load(fileoutput)

updated_file = open('ignored_skus.txt', 'r')
updated_skus = updated_file.read().split('\n')

# fileoutput = open('reverseStockx.txt', 'r')
# links = fileoutput.read().split('\n')
# links = [p.split('*')[0].lower() for p in links if p]
...
def get_number(size):
    res = [v for v in size if v.isdigit() or v == '.' or v == 'O' or v == 'S']
    res = ''.join(res)
    return res
...
class StockX(scrapy.Spider):
            ...
            for gender in genders:
                ...
                while i <= 1:
                    yield scrapy.Request(
                        f'https://stockx.com/api/browse?productCategory=sneakers&gender={gender}&brand={brand}&page={i}&dataType=product',
    ...
    def parse(self, response, **kwargs):
        ...
        for product in data['Products']:
            if [v for v in linksDictionar if product['shortDescription'].lower() in v.split('*')[-1].lower()].__len__() != 0:
                link = 'https://stockx.com/api/products/{}?includes=market,360&currency=USD&country=AE'.format(
                    product['shortDescription'])
                yield scrapy.Request(link, callback=self.parse_product, meta={'datajson': product, 'link': link,
                                                                              'oldsku':[v for v in linksDictionar if product['shortDescription'].lower() in v.split('*')[-1].lower()][0].split('*')[0],
                                                                              'availablesizes': linksDictionar[[v for v in linksDictionar if product['shortDescription'].lower() in v.split('*')[-1].lower()][0]]},
                                     headers=self.headers)
            else:
                link = 'https://stockx.com/api/products/{}?includes=market,360&currency=USD&country=AE'.format(
                    product['shortDescription'])
    ...
    def parse_product(self, response):
        try:
            ...
            yield scrapy.Request(response.meta['link'], headers=self.headers, dont_filter=True, meta={
                'datajson': response.meta['datajson'], 'link': response.meta['link']
            })
            return
        ...
        if response.meta['oldsku'] !='':
            item['group_sku'] = response.meta['oldsku']
        else:
            item['group_sku'] = product['styleId'].replace("'", "") + '-' + item['color']
        # converted from ticker symbol to style id

        # item['group_sku'] = response.meta['skuold']     # product['tickerSymbol'] + '-' + item['color']
        ...
        if not images or images[0] == None:
            ...
            print('no 360 {}'.format(response.url))
        else:
            print('success...')
        item['main_pic'] = images[0].split('?')[0]
        if is_image_360:
            item['pic1'] = images[5].split('?')[0]
            item['pic2'] = images[10].split('?')[0]
            item['pic3'] = images[15].split('?')[0]
            item['pic4'] = images[20].split('?')[0]
            item['pic5'] = images[25].split('?')[0]
            # for index, image in enumerate(images):
            #     if index > 34:
            #         continue
            #     item['pic{}'.format(index + 1)] = image.split('?')[0]
            # index = 0
        else:
            for inx, image in enumerate(images):
                if inx > 4:
                    continue
                item['pic{}'.format(inx + 1)] = image.split('?')[0]
        if response.meta['oldsku'] == '':
            sizeofsku = 0
        else:
            sizeofsku = response.meta['oldsku'].split('-').__len__()
        index = 0
        for variant in (list(product['children'].values())):
            index += 1
            # if response.meta['availablesizes'] ==[]:
            # if variant['market']['lowestAsk'] == 0:
            #     continue
            if variant['shoeSize'] == '1':
                continue
            if variant['shoeSize'] != '':
                item['variant_{}'.format(index)] = variant['shoeSize']
            else:
                for tr in variant['traits']:
                    if tr['name'] == 'Color':
                        item['variant_{}'.format(index)] = tr['value']
                        break
            try:
                item['variant_{}_price'.format(index)] = int(variant['market']['lowestAsk'])

            except:
                item['variant_{}_price'.format(index)] = 0
            if response.meta['availablesizes'] !=[]:
                try:
                    if [v for v in response.meta['availablesizes'] if get_number(v.split('-')[sizeofsku:][0]) == get_number(item['variant_{}'.format(index)])][0]:
                        continue
                except:
                    if variant['shoeSize'] == '1':
                        continue
            ...
            ditem = dict()
            ditem['ref'] = TS
            ditem['name_en'] = item['name_en']
            ditem['brand'] = item['brand']
            ditem['gender'] = item['gender']
            ditem['partner_id'] = 1364
            # ditem['url_key'] = 'test.com/sneakers'
            ditem['group_sku'] = item['group_sku']
            ditem['product_sku'] = item['group_sku'] + '-' + item['variant_{}'.format(index)]
            ditem['description_en'] = item['description_en']
            ditem['description_plain_en'] = item['description_plain_en']
            ditem['main_category'] = item['main_category']
            ditem['category'] = item['category']
            ditem['sub_category'] = item['sub_category']
            ditem['base_category'] = item['base_category']
            ditem['color'] = item['color']
            ditem['size_slug'] = item['size_slug']
            ditem['size'] = item['variant_{}'.format(index)]
            ditem['delivery_information'] = '15 to 20 days'
            ditem['main_pic'] = item['main_pic']
            ditem['pic1'] = item['pic1']
            ditem['pic2'] = item['pic2']
            ditem['pic3'] = item['pic3']
            ditem['pic4'] = item['pic4']
            ditem['pic5'] = item['pic5']
            ditem['whole_sale_price'] = item['variant_{}_price'.format(index)]
            if 'AED' in item['serial_no']:
                processed.append(ditem)
            # if 'AED' in item['serial_no']:
            #     for i in range(0, 3):
            #         PUitem = dict()
            #         PUitem['sku'] = product['tickerSymbol'].replace("'", "") + '-' + item['color'] + '-' + item['variant_{}'.format(index + 1)]
            #         # PUitem['sku'] = item['group_sku'] + '-' + ''.join([v for v in item['variant_{}'.format(index + 1)] if v.isdigit() or v == '.'])
            #         if i == 0:
            #             PUitem['store_view_code'] = 'en'
            #             PUitem['price'] = int((50 + (item['variant_{}_price'.format(index + 1)] * 1.13)) * 4.38)
            #             PUitem['status'] = 1
            ...
            #             PUitem['price'] = int(((((50 + (item['variant_{}_price'.format(index + 1)] * 1.13)) * 4.38) * 10) / 100) + (50 + (item['variant_{}_price'.format(index + 1)] * 1.13)) * 4.38)
            #             PUitem['status'] = 1
            #             if item['variant_{}_price'.format(index + 1)] == 0:
            #                 PUitem['price'] = 0
            #                 PUitem['status'] = 2
            ...
            #             PUitem['price'] = int(((50 + (item['variant_{}_price'.format(index + 1)] * 1.13)) * 4.38)/3.67)
            #             PUitem['status'] = 1
            #             if item['variant_{}_price'.format(index + 1)] == 0:
            #                 PUitem['price'] = 0
            #                 PUitem['status'] = 2
            #         pricewritermagentonew.writerow(PUitem)
            #         priceupdatemagentonew.flush()
            #
            #     PLitem = dict()
            #
            #     PLitem['GROUP SKU'] = product['tickerSymbol'].replace("'", "") + '-' + item['color']
            #     PLitem['PRODUCT SKU'] = product['tickerSymbol'].replace("'", "") + '-' + item['color'] + '-' + item['variant_{}'.format(index + 1)]
            #
            #     # PLitem['GROUP SKU'] = item['group_sku']
            #     # PLitem['PRODUCT SKU'] = item['group_sku'] + '-' + ''.join([v for v in item['variant_{}'.format(index + 1)] if v.isdigit() or v == '.'])
            #     PLitem['RETAIL PRICE'] = int((50 + (item['variant_{}_price'.format(index + 1)] * 1.13)) * 4.38)
            #     if item['variant_{}_price'.format(index + 1)] == 0:
            #         PLitem['RETAIL PRICE'] = 0
            #     PLitem['WEBSITES'] = 'United Arab Emirates'
            #     pricelistwriterodoonew.writerow(PLitem)
            #     pricelistodoonew.flush()


        # item['alreadyexists'] = alreadyexists

        # writerstockx.writerow(item)
        # csvfile.flush()

    def close(spider, reason):
        vandor = 'SNEAKERS STOCK | DS'
        obj.insert_records(items = processed, vandor = vandor)
    #     priceupdatemagentonew.close()
    #     pricelistodoonew.close()
    #     patch = csvdiff.diff_files('stockx_pricelist_magento_new_original.csv', 'stockx_pricelist_magento_new_temp.csv',
    #                                ['sku', 'store_view_code'])
    #     '''delta file for magento'''
    #     deltaupdateheaders = ['sku', 'price', 'status', 'store_view_code']
    #     deltapriceupdate = open('stockx_pricelist_magento_new_delta.csv', 'w', newline='', encoding='utf-8')
    #     deltapricewriter = csv.DictWriter(deltapriceupdate, fieldnames=deltaupdateheaders)
    #     deltapricewriter.writeheader()
    #
    #     originalpriceupdate = open('stockx_pricelist_magento_new_original.csv', 'a+', newline='', encoding='utf-8')
    ...
    #     original_list = list(csv.DictReader(open('stockx_pricelist_magento_new_original.csv')))
    #     originalpriceupdate = open('stockx_pricelist_magento_new_original.csv', 'w', newline='', encoding='utf-8')
    #     originalpriceupdatewriter = csv.DictWriter(originalpriceupdate, fieldnames=deltaupdateheaders)
    ...
    #     ####        odoo code         ####
    #
    #     patch_2 = csvdiff.diff_files('stockx_pricelist_odoo_new_original.csv', 'stockx_pricelist_odoo_new_temp.csv',
    #                                  ['GROUP SKU', 'PRODUCT SKU', 'WEBSITES'])
    #     '''delta file for pricelist'''
    #     deltapricelistheaders = ['GROUP SKU', 'PRODUCT SKU', 'RETAIL PRICE', 'WEBSITES']
    #     deltapricelistfile = open('stockx_pricelist_odoo_new_delta.csv', 'w', newline='', encoding='utf-8')
    #     deltapricelistwriter = csv.DictWriter(deltapricelistfile, fieldnames=deltapricelistheaders)
    #     deltapricelistwriter.writeheader()
    #
    #     originalpricelist = open('stockx_pricelist_odoo_new_original.csv', 'a+', newline='', encoding='utf-8')
    ...
    #         changed.append(item_)
    #
    #     original_list = list(csv.DictReader(open('stockx_pricelist_odoo_new_original.csv')))
    #     originalpriceupdate = open('stockx_pricelist_odoo_new_original.csv', 'w', newline='', encoding='utf-8')
