...
csv_columns = ['serial_no', 'ref', 'partner_id', 'product_type', 'group_sku', 'variation_type', 'product_sku', 'upc_ean', 'barcode', 'brand',
...
csvfile = open("sunglasses2u.csv", 'w', newline='', encoding="utf-8")
writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
writer.writeheader()

class Sunglasses2u(scrapy.Spider):
    name= "sunglasses2u"
    custom_settings = {
    ...
    def start_requests(self):
        yield scrapy.Request(url="https://www.sunglasses2u.com/")

    def parse(self, response):
        for genders in response.css("ul.area_top_dds li.area_top_dds_mi.area_top_dds_mi_vis_both"):
            gender = genders.css("a.area_top_dds_link span::text").get()
            if "Mens Sunglasses" in gender or "Ladies Sunglasses" in gender or "Kids" in gender:
                g = genders.css("a.area_top_dds_link::attr(href)").get()
                yield scrapy.Request(url =g,callback=self.data)
        yield scrapy.Request(url="https://www.sunglasses2u.com/ladies-sunglasses.html?page_num=3",callback=self.data)
        yield scrapy.Request(url="https://www.sunglasses2u.com/mens-sunglasses.html?page_num=3",callback=self.data)
    ...
    def data(self,response):
        g = 7
        for glasses in response.css("div.xcomponent_products_container div.xcomponent_products_medium"):
            links = glasses.css("a.xcomponent_products_medium_link::attr(href)").get()
            yield scrapy.Request(url=links,callback=self.scrape_data)
        total_pages = int(response.css("div.page_search_results_subbar_right span::text").get().split("of")[1])
        for l in range(1, total_pages):
            url = response.url.split("?")[0] + "?page_num=" + str(l)
            yield scrapy.Request(url=url, callback=self.data)

    def scrape_data(self,response):
        item =dict()
        count = 1
        for var in response.css("div.page_products_details5_bottom_pane_specs_col table tr td"):
            if "Frame Colour" in var.css("::text").get():
                item["color"] = var.xpath("following-sibling::td/text()").get()
        sku = response.css("div.page_products_details5_bottom_pane_text span.shared_title_4::text").get().split()[0]
        # try:
        #     item["group_sku"] = sku + "-" + item["color"]
        # except:
        #     item["group_sku"] = sku
        item["group_sku"] = response.url
        item["size_slug"] = "EU"
        item["name_en"] = " ".join([z for z in response.css("h1.page_products_details5_top_h1 span::text").getall()])
        item["origin"] = "sunglasses2u"

        item["delivery_information"] = "10-15 days"
        item["brand"] = response.url.split("/")[4]
        item["retail_price"] = "".join([i for i in response.css("div.page_products_details5_top_price_1 ::text").getall()])
        item["whole_sale_price"] = "".join([i for i in response.css("div.page_products_details5_top_price_1 ::text").getall()])
        item["description_plain_en"] = "\n".join([t for t in response.css("div.page_products_details5_top_description ::text").getall()])
        item["gender"] = ",".join([h.split(":")[1] for h in response.css("div.page_products_details5_bottom_pane_specs_col table tr td div::attr(data-title)").getall() if "Gender" in h])
        item["main_category"] = item["gender"]
        item["category"] = "glasses"
        item["sub_category"] = "sunglasses"
        item["ref"] = ts
        item["base_category"] = item["gender"]+"/" +item["category"] +"/" +item["sub_category"]
        item["description_en"] = "\n".join([t for t in response.css("div.page_products_details5_top_description ::text").getall()])
        item["original_price"] = "".join([i for i in response.css("div.page_products_details5_top_price_1 ::text").getall()])
        item["main_pic"] = response.css("div.page_products_details5_top_images input#page_products_details5_large::attr(value)").get()
        for images in response.css("div#page_products_details5_options a img::attr(src)").getall():

            item["pic_"+str(count)] = images
            count = count + 1
        item["strike_through_price"] = "".join([a.strip() for a in response.css("span.text strike ::text").getall()])
        count_2 = 1
        if  response.css("div.page_products_details5_top_switches2 div a") != []:
            for variant in response.css("div.page_products_details5_top_switches2 div a"):
                vari = variant.css("::attr(onmouseover)").get().split("(")[-1].replace(";","").replace("'","").replace(")","").strip().lower()
                item["variant_" + str(count_2)] = vari
                item["variant_" + str(count_2) + "_stock"] = response.css("div.stockin::text").get()
                item["variant_" + str(count_2) + "_price"] = "".join(
                    [i for i in response.css("div.page_products_details5_top_price_1 ::text").getall()])
                count_2 = count_2 + 1
        else:
            pass
        s = 7
