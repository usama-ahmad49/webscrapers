...
class coltortiboutique(scrapy.Spider):
    ...
    def final_page(self,response):
        ...
        if item['color'] is not None:
            ...
            item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', str(response.css('div.product-info-main div.product.attribute.sku div.value::text').get()) + '-' + item['color'])
        else:
            item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', response.css('div.product-info-main div.product.attribute.sku div.value::text').get())
