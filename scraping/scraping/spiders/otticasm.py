...
cs = open('otticasm.csv', 'w', newline="", encoding='utf-8')
...
class otticasm(scrapy.Spider):
    ...
    def start_requests(self):
        url = 'https://www.otticasm.com/occhiali-da-sole.html'
    ...
    def parse(self, response, **kwargs):
        for res in response.css('.products-grid.columns3.hide-addtolinks.hide-addtocart li'):
            ...
            yield scrapy.Request(url=lnk, callback=self.parse_data)
    ...
    def parse_data(self, response, **kwargs):
        ...
        item['original_price'] = response.css('.product-info .price-box .special-price .price::text').extract_first().strip().replace(',', '.')
        ...
        try:
            for var in response.css('.products-grid.columns4')[0].css('li'):
                if index <= 100:
                    item[f'variant_{index}'] = var.css('.list-sku ::text').extract_first()
                    item[f'variant_{index}_price'] = var.css('.action-area .special-price .price::text').extract_first().strip()
