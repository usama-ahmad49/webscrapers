# div.react--explore.tw.tw-font-sans.tw-w-full a.tw-flex.tw-flex-col.tw-justify-center.tw-py-4.tw-text-center.tw-cursor-pointer.tw-no-underline.tw-px-8
# https://api.blickers.com/products/?cursor=26&locale=en_GB&country=PK&currency=EUR&shop=blickers.com


# vitkac_cart_unlogged_token=841c9e00e2185dbe4553c3a67436ecd846e37abe90e0dca809053cbf629ed0a3; vtk_user_culture=en; _gcl_au=1.1.1823859536.1632910474; __exponea_etc__=0ef6d6f9-8c60-4836-8bf5-14ae888d2933; _gid=GA1.2.1126468611.1632910474; _vwo_uuid_v2=DED65C7089CCB552F4E3FAD7207B28CCD|1bdf5bdbb4eceeb7e56300f5097765c6; rmStore=amid:43243; __exponea_ab_product_rcm_ABtest__=A; rskxRunCookie=0; rCookie=qzjsumv1vh8oclxvsjddbhku5hpbx9; frnt_sess=8a53079a370c1cc152e5001c53f8980b; __exponea_time2__=0.7676703929901123; mainCategory={"main_category":"277","sub_category":"accessories","last_category":"glasses-1"}; stc115050=env:1632986685%7C20211031072445%7C20210930080246%7C17%7C1045882:20220930073246|uid:1632910474573.951797437.3575721.115050.1483544135.:20220930073246|srchist:1045882%3A1632986685%3A20211031072445:20220930073246|tsa:1632986685867.256730648.7577076.5874761804922946.1:20210930080246; _uetsid=0b478560210e11ec9d19f546f4012976; _uetvid=0b47a0a0210e11ecae8b3f9fca0fb739; _ga_TTHRVCLRZC=GS1.1.1632986628.4.1.1632987166.18; _ga_RBFMS85MER=GS1.1.1632986628.4.1.1632987166.18; _ga=GA1.2.5515599.1632910474; cto_bundle=uQwqYF9YWGM4RXR2bkl0aUd2dkRrdWZFbDNQNkRkTmtsMVNBT2h6QWw5M3lMclZiaDFDNmV3Tmp3TlJxSEtGbXRYcXQyaTFxajZncU44TFptVWslMkZoUyUyQlpMeE5wS0hnZzVUQnJvN2U2MzkzZnE1Y3drUXVxQW1Heklpd3cxejMlMkZMTUNVTWlPRTJsOUVYbG1IMWRqZWtVZlZueEElM0QlM0Q; lastRskxRun=1632987175765
...
cs = open('blickers.csv', 'w', newline="", encoding='utf-8')
...
class blickers(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # yield scrapy.Request(url='https://api.blickers.com/products/?gender=women%2Cunisex&cursor=13&locale=en_GB&country=PK&currency=EUR&shop=blickers.com',meta={'gender': 'Women'})


        i=0
        while(i<=10000):
            # yield scrapy.Request(url=f'https://api.blickers.com/products/?cursor={i}&locale=en_GB&country=PK&currency=EUR&shop=blickers.com',callback=self.next_page)
            yield scrapy.Request(url=f'https://api.blickers.com/products/?gender=women&cursor={i}&locale=en_GB&country=PK&currency=EUR&shop=blickers.com',meta={'gender': 'Women'},callback=self.next_page)
            yield scrapy.Request(url=f'https://api.blickers.com/products/?gender=men&cursor={i}&locale=en_GB&country=PK&currency=EUR&shop=blickers.com',meta={'gender': 'Men'},callback=self.next_page)
    ...
    def next_page(self,response):
        ...
        for results in json.loads(json_string)['results']:
            product_page=results['uri']
            yield scrapy.Request(url='https://www.blickers.com/en/glasses/'+product_page,callback=self.final_page,meta={'gender':response.meta['gender']})


    def final_page(self,response):
        item=dict()
        item['gender']=response.meta['gender']
        item['name_en']=''.join(response.css('h1.tw-text-dark.tw-text-base.tw-font-medium.tw-flex.tw-truncate span::text').getall())



        # for gender in response.css("dl.tw-text-sm")[1].css("div.tw-flex.tw-w-full.tw-items-end.tw-overflow-x-hidden"):
        #     if gender.css("dt::text").get().strip() == "Gender":
        #         genders = [i.strip() for i in gender.css("dd a::text").getall()]
        #         if len(genders) ==1:
        #             ge = genders
        #             item['gender'] = ''.join(ge)
        #         else:
        #             ge = ",".join([v for v in genders])
        #             item['gender']=ge
        ...
        jsn_string2=response.css('div#summary-product-variations::attr(data-variations)').get()
        index = 1
        for key,value in enumerate(json.loads(jsn_string2)):

            if key==0:
                t=1
                for k in value['image_urls'].split(','):
                    if t < 37:
                        if value != None:
                            item['pic_' + str(t)] = k
                            item['brand'] = value['visible_brand']
                            item['original_price']=value['price']
                            item['retail_price']=value['price']
                            item['whole_sale_price']=value['price']
                            item['category']=value['subtype_key']
                            item['sub_category']=value['subtype_key']
                            item['color']=value['variation_name']
                            item['group_sku']=str(value['model'])+str('-')+str(item['color'])
                            ...
                            t = t + 1
            if index<100:
                item[f'variant_{index}'] =value['variation_name']
                item[f'variant_{index}_price'] =value['price']
