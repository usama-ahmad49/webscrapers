...
cs = open('swisswatchesforsale.csv', 'w', newline="", encoding='utf-8')
...
class swisswatchesforsale(scrapy.Spider):
    name = 'swisswatchesforsale'
    def start_requests(self):
        yield scrapy.Request('https://swisswatchesforsale.com/product-category/watches/')
    def parse(self, response, **kwargs):
        totalpages = int(response.css('.page-numbers .page-numbers')[-2].css('a::text').extract_first())
        for page in range(1, totalpages + 1):
            yield scrapy.Request(f'https://swisswatchesforsale.com/product-category/watches/page/{page}/', callback=self.parse_data, dont_filter=True)

    def parse_data(self, response):
        for link in response.css('.item-list h3 a::attr(href)').extract():
            yield scrapy.Request(url=link, callback=self.parse_product)

    def parse_product(self, response):
        item = dict()
        item['ref'] = ts
        item['group_sku'] = response.css('.sku::text').extract_first()
        item['name_en'] = response.css('h1::Text').extract_first()

        try:
            item['strike_through_price'] = response.css('.summary.entry-summary .price del span::Text').extract()[-1]
        except:
            pass
        try:
            item['original_price'] = response.css('.summary.entry-summary .price ins span::Text').extract()[-1]
            item['retail_price'] = response.css('.summary.entry-summary .price ins span::Text').extract()[-1]
            item['whole_sale_price'] = response.css('.summary.entry-summary .price ins span::Text').extract()[-1]
        except:
            item['original_price'] = 'price on request'
            item['retail_price'] = 'price on request'
            item['whole_sale_price'] = 'price on request'

        for var in response.css('.woocommerce-product-attributes.shop_attributes tr'):
            if 'Brand' in var.css('th::text').extract_first():
                item['brand'] = var.css('td ::text').extract_first()
            elif 'Dial Color' in var.css('th::text').extract_first():
                item['color'] = var.css('td ::text').extract_first()
            elif 'Band Material' in var.css('th::text').extract_first():
                item['material'] = var.css('td ::text').extract_first()
            elif 'Gender'in var.css('th::text').extract_first():
                item['gender'] = var.css('td ::text').extract_first()
            elif 'Condition' in var.css('th::text').extract_first():
                item['condition'] = var.css('td ::text').extract_first()
            elif 'Case Size' in var.css('th::text').extract_first():
                item['size'] = var.css('td ::text').extract_first()
                item['size_slug'] = ''.join(list(var.css('td ::text').extract_first())[2:])
        try:
        ...
        item['main_pic'] = response.css('.woocommerce-main-image::attr(href)').extract_first()
        item['pic_1'] = item['main_pic']
        item['description_plain_en'] = ' '.join(response.css('.woocommerce-product-details__short-description ::text').extract())
        item['description_en'] = ' '.join(response.css('.woocommerce-product-details__short-description ::text').extract())
        item['variant_1'] = response.css('.sku::text').extract_first()
...
process.crawl(swisswatchesforsale)
process.start()