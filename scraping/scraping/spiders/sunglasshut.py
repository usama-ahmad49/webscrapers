import csv
# sold out https://www.sunglasshut.com/us/prada/pr01vs-8053672987447
import datetime
import json
import time
import re
...
cs = open('sunglasshut.csv', 'w', newline="", encoding='utf-8')
...
class sunglasshut(scrapy.Spider):
    name = 'sunglasshut'

    def start_requests(self):
        urls = ['https://www.sunglasshut.com/us/womens-sunglasses', 'https://www.sunglasshut.com/us/mens-sunglasses', ]
        for url in urls:
            count = 2
            ...
            total_products = driver.find_element_by_css_selector('#skip-nav-contents > div:nth-child(2) > div.sgh-load-more.container.flex.flex-col.h-full.justify-center.items-center.content-center.py-20 > p').text.split()[-1]
            total_pages = int(total_products) / 18
            driver.close()
            ...
            yield scrapy.Request(url=url, callback=self.parse, meta={'gender': gender, 'pages': total_pages, 'count': count})

    def parse(self, response):

        for links in response.css('article.sgh-tile.container.sh-loaded a::attr(href)').getall():
            yield scrapy.Request(url='https://www.sunglasshut.com' + links, callback=self.next_page, meta={'gender': response.meta['gender'], 'pages': response.meta['pages']})

        if response.meta['count'] <= response.meta['pages']:
            value = response.url.split('?')[0] + '?currentPage='
            yield scrapy.Request(url=value + str(response.meta['count']), callback=self.parse, meta={'gender': response.meta['gender'], 'pages': response.meta['pages'], 'count': response.meta['count'] + 1})
    ...
    def next_page(self, response):
        ...
        item = dict()
        item['ref'] = ts
        jsonstring = response.css('script[type="application/ld+json"]::text').get()
        try:
            jsonData = json.loads(jsonstring)
        except:
            try:
                jsonData = json.loads(jsonstring.replace('""', '"'))
            except:
                b = 1
        if b == 0:

            item['brand'] = jsonData['brand']
            item['name_en'] = response.css('div.sgh-breadcrumb__levels span::text').get().strip()
            item['original_price'] = jsonData['offers']['price']
            item['retail_price'] = item['original_price']
            item['whole_sale_price'] = item['original_price']
            item['main_pic'] = jsonData['image']
            if response.css('div.sgh-pdp__filter-description.sgh-pdp__filter-description--colors span.sgh-pdp__filter-value::text').get() is not None:
                item['color'] = response.css('div.sgh-pdp__filter-description.sgh-pdp__filter-description--colors span.sgh-pdp__filter-value::text').get().strip().replace('\r\n\t\t\t\t', '')
            try:
                item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', (jsonData['sku']+'-'+item['color']))
            except:
                item['group_sku'] = re.sub('[^A-Za-z0-9-.\s+]+', '', (jsonData['sku']))
            ...
            item['origin'] = 'Website'
            item['size_slug'] = 'EU'
            if response.css('span.original-price::text').get() is not None:
                item['strike_through_price'] = response.css('span.original-price::text').get().strip()[1:]
            # item['size']=response.css('div.sgh-pdp__filter-description.sgh-pdp__filter-description--sizes span.sgh-pdp__filter-value::text').get().strip().replace('\r\n\t\t\t','')
            if response.css('p.panel-description root::text').get() is not None:
                item['description_en'] = response.css('p.panel-description root::text').get()
                item['description_plain_en'] = item['description_en']

            t = 1
            for j in response.css('div.pdp-slide-menu-body.js-pdp-slide-menu-body li a div.pdp-slide-menu-filter-list-preview'):
                if t < 37:
                    item['pic_' + str(t)] = j.css('img::attr(src)').get()
                    t = t + 1
                else:
                    break

            index = 1
            for var in response.css('div.sgh-pdp__filter-description.sgh-pdp__filter-description--sizes span.sgh-pdp__filter-value::text').getall():
                item[f'variant_{index}'] = var.strip().replace('\r\n\t\t\t', '')
                item[f'variant_{index}_price'] = item['original_price']
                if response.css('div[data-element-id="X_Pdp_Prod_AddCart"] div.out-of-stock.hideBtn p::text').get() is not None:
