...
fileoutput = open('imagesprobelmsku_temp.txt', 'r')
...
headers_csv = ['serial_no','url_key','group_sku','ticker','brand', 'name_en','color', 'main_pic',
...
csvfile = open('stockx_data_image.csv', 'w', newline='', encoding='utf-8')
...
class StockX(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        while index<len(images):
            item['pic_{}'.format(jdex + 1)] = images[index].split('?')[0]
            if img360:
                index += 6
            else:
                index +=1
