...
csvfile = open('tapologyspider.csv', 'w', newline='', encoding="utf-8")
...
class tapologyspider(scrapy.Spider):
    ...
    start_url = ['https://www.tapology.com/fightcenter']
    ...
    def start_requests(self):
        ...
        for fight_data in [v for v in list(set(data.split('\n'))) if v]:
            ...
            yield scrapy.Request(url=link, callback=self.parse, headers=headers_3, meta={
                'proxy': 'https://5.79.73.131:13010', 'name': athlete_name})

    # def parse(self, response):
    #     for FightLinks in response.css('fcListing .billing a::attr(href)').extract():
    #         yield scrapy.Request('https://www.tapology.com/fightcenter{}'.format(FightLinks), callback=self.parseFight)

    def parse(self, response):
        item = dict()
        item['name_on_ufc'] = response.meta['name']
        ...
        item['left_fighter'] = response.css('.fighterNames.botPad.clearfix .fName.left a ::text').extract()
        item['right_fighter'] = response.css('.fighterNames.botPad.clearfix .fName.right a ::text').extract()
