...
csvfile = open('stockx_data_images.csv', 'w', newline='', encoding='utf-8')
...
class StockX(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for product in data['Products']:
            if product['shortDescription'].lower() in links:
                continue
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            pass
        item['group_sku'] = product['tickerSymbol'].replace("'", "") + '-' + item['color']  # response.meta['skuold']
        ...
        for variant in (list(product['children'].values())):
            if item['group_sku'] + '-' + variant['shoeSize'] in updated_skus:
                alreadyexists = 1
                continue
