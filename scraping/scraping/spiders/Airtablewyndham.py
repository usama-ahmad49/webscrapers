...
class AirtableClient:
    ...
    def search_record(self,table_name, searchurl):
        global iid
        url = BASE_URL + '/' + self.base_key + "/" + table_name
        querystring = {"filterByFormula": "({Application ID}=" + f"\"{searchurl}\")"}
    ...
    def insert_records(self, table_name, records):
        ...
        for chunk in self.chunks(items=records, size_of_chunks=10):
            ...
            dat = {"records": []}
            for item in chunk:
                exists = self.search_record(table_name,item['Application ID'])
                if exists == True:
